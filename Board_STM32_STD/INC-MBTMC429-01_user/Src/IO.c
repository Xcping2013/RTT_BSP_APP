...
void pressureAlarm_Handler(void)
{
	...
	{			
		...
		pressureAlarm=1;
		UART_Printf("stop motor z due to pressure overhigh!!!\r\n>>");	
	}
	else pressureAlarm=0;	
}
//增加调试打印命令
...
void releaseAlarmTask(void)
{
  	if(abs((int)(GetSysTimer()-task1Delay))>200)
  	{
//			if(INPUT8==1)	pressureAlarm=0;
			//if(printStr5times!=0) 
			if(pressureAlarm==1)
			{
				//printStr5times--;
