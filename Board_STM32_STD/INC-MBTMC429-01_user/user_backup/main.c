  ...
  InitUART(baud);
	...
	switch(AT24CXX_ReadOneByte(ADDR_PROJECT_ID))
	{
		...
		case BUTTON_OFFLINE:	ProID=BUTTON_OFFLINE, RampInit_Button();	UART_SendStr(" Fixture Type=BUTTON OFFLINE\n>>");
				break ;
		case BUTTON_VER3:			ProID=BUTTON_VER3, 		RampInit_Button();	UART_SendStr(" Fixture Type=BUTTON_VER3\n>>"); //EXTIX_Init();
				break ;
		case BUTTON_ROAD:			ProID=BUTTON_ROAD,		RampInit_Button();	UART_SendStr(" Fixture Type=BUTTON ROAD\n>>");		
				break ;
		...
		case JABIL_LCR:				ProID=JABIL_LCR,		  SW_init_JABIL_LCR();UART_SendStr(" Fixture Type=JABIL LCR\n>>");	RGB_B_ON();	
				break ;
		case BASIC_APP:				ProID=BASIC_APP,													UART_SendStr(" Fixture Type=IAC LCR\n>>");
				break ;
