...
uint8_t  CommmandLine_speed(char *Commands)
{
	if(strncmp(Commands+1,"# velocity ",11)==0 ) 
  {
			...
			if(0<len&&len<=4)			
      {
				...
				if(ProID==BUTTON_ONLINE||ProID==BUTTON_OFFLINE ||ProID==BUTTON_VER3)
				{
...
uint8_t  CommmandLine_home(char *Commands)
{
	if(strncmp(Commands+1,"# search home from left to right ",33)==0 ) 
  {
		...
		if(ActualMotor<N_O_MOTORS)
		{
			...
			if(0<len&&len<=4)			
      {
				SpeedForHoming[ActualMotor]=strtol(s, &p, 10);	
				...
				SetAmaxAuto(ActualMotor, SpeedForHoming[ActualMotor]);
				RotateLeft(ActualMotor,SpeedForHoming[ActualMotor]);
