...
u16  SpeedForHoming[3]={839,839,839};
...
void KeyRESET_EXE(void)
{
	if( INPUT2==0 && KeyRESET_ON==0)					//第一次按键有效
	{
		...
		if( INPUT2==0 )													//去抖动
		{														
			KeySTART_ON=0;RGB_B_ON();	OUTPUT1=0;	//解除开始自锁,控制指示灯			
...
{
	KeyRESET_ON=1;	OUTPUT2=1;	MotorIsHomed=0;															
	//设置回原点速度模式下的最大速度和加速度
	Write429Int(IDX_VMAX|MOTOR0, 2047);			SetAMax(0, 1000);
	Write429Int(IDX_VMAX|MOTOR1, 2047);			SetAMax(1, 1000);
	Write429Int(IDX_VMAX|MOTOR2, 2047);			SetAMax(2, 1000);
	...
	switch(ID)
	{
		...
		case BUTTON_ONLINE:
				 HomeStep=Y_GoHome;
				 RotateLeft(AXIS_Y,SpeedForHoming[AXIS_Y]);	//逆时针旋转		L		Y轴先复位		Y轴先后后前
				 break;
		case BUTTON_VER3:
		...
		case BUTTON_ROAD:
				 HomeStep=Z_GoHome;
				 RotateLeft(AXIS_Z,SpeedForHoming[AXIS_Z]);	//逆时针旋转		L		Z轴先复位		Z轴先上后下
				break;	
		default:
			break;
	}
...
void MotorHoming_WithHomeSensor( uint8_t step)
{
	...
	{	
		...
		switch(step)
		{
				case X_GoHome:	
					...
					if(X_HomeSensor==1 && (Read429Short(IDX_VACTUAL|(0<<5)))<0)			
					{
						SetUpAfterHomed(AXIS_X);	step=GoHome_OK;
						UART_Printf("Axis X Homed<OK>\n>>");								
					}
					if((SwitchStatus& (0x02<<AXIS_X*2)) ? 1:0)//REFL1
					{																																					 //硬件限位自动停止
						RotateRight(AXIS_X,SpeedForHoming[AXIS_X]);													 //顺时针 正转 R
						if(SensorCheck[0]==1) SensorCheck[0]=2;  
					}
					if((SwitchStatus& (0x01<<AXIS_X*2)) ? 1:0)//REFR1
					{
				...
				case Y_GoHome:	
					...
					if((Y_HomeSensor==1 && (Read429Short(IDX_VACTUAL|(1<<5))))>0)		//Y轴到达原点位置且未复位过
					{
						SetUpAfterHomed(AXIS_Y);	step=Z_GoHome;		RGB_B_ON ();				  		
						UART_Printf("Axis Y Homed<OK>\n>>");													//打印提示	
						RotateLeft(AXIS_Z,SpeedForHoming[AXIS_Z]);									  //Z轴先上后下			
					}
					...
					break;
			 case Z_GoHome:
					if((Z_HomeSensor==1)&& (Read429Short(IDX_VACTUAL|(2<<5)))>0 )
					{
						SetUpAfterHomed(AXIS_Z);	step=X_GoHome;	 RGB_B_ON ();
						...
						RotateRight(AXIS_X,SpeedForHoming[AXIS_X]);			//X轴先左后右		
					}
				 ...
				 break;
			 case GoHome_OK:
						KeyRESET_ON=0;	OUTPUT2=0;	KeySTART_ON=0;	OUTPUT1=0;	RGB_G_ON(); MotorIsHomed=1;
...
void KeyRESET_EXE_OFFLINE(void)
{
	if( INPUT2==0 && KeyRESET_ON==0)			//第一次按键有效
	{
		...
		if( INPUT2==0 )													//去抖动
		{
			KeySTART_ON=0;										  //解除开始自锁			
			OUTPUT1=0;
			if(ProID==BUTTON_OFFLINE)			{Cylinder_Reset_check();	MotorHoming();}
//			if(ProID==BUTTON_VER3 )			{MotorHoming_ver3();}			

			if(ProID==BUTTON_VER3 && INPUT3==1 && INPUT4==0)			{MotorHoming_ver3();}			
			else {KeySTART_ON=1;	OUTPUT1=1;}	
		}
	}	
}
//
...
//设置回原点速度模式下的最大速度和加速度
	...
	SensorCheck[0]=0;	SensorCheck[1]=0;	SensorCheck[2]=0;
	RotateLeft(AXIS_Z,SpeedForHoming[AXIS_Z]);		//逆时针旋转		L		
	MotorIsHoming[AXIS_Z]=1;
}
	...
	if(KeyRESET_ON==1)													//回原点使能
	{
		...
		if((MotorIsHoming[AXIS_Y]==1)&&(INPUT6==1))	//Y轴到达原点位置且未复位过
		{
			if((Read429Short(IDX_VACTUAL|(1<<5)))>0)	  //Y轴回原点方向正确			
			{
				...
				RGB_B_ON ();	
				MotorIsHoming[AXIS_Y]=0;									//Y轴结束回原点
				MotorIsHoming[AXIS_X]=1;									//Z轴开始回原点
				RotateRight(AXIS_X,SpeedForHoming[AXIS_X]);
			}
		}
...
{
	MotorIsHomed=0;	KeyRESET_ON=1;	OUTPUT2=1;	
	Write429Int(IDX_VMAX|MOTOR2, 2047);								
	SetAMax(2, 1000);
	UART_Printf("motor homing...\n");
	SensorCheck[2]=0;	
...
void MotorHoming_Road(void)
{
	if(KeyRESET_ON==1)														//回原点使能
	{
		if(INPUT3==1)
		{
			...
			static U32_T SensorDelay;
			if(DoOnce==0) {SensorDelay=GetSysTimer(); DoOnce=1;}
			if((MotorIsHoming[AXIS_Z]==1)&&(INPUT4==1))
			{
				if((Read429Short(IDX_VACTUAL|(2<<5)))>0)
				{
					SetUpAfterHomed(AXIS_Z);	
	...
	//				Set429RampMode(2, RM_RAMP);	
			...
			if(MotorIsHoming[AXIS_Z]==0) //所有轴回原点完成
			{
				KeyRESET_ON=0;	OUTPUT2=0;	MotorIsHomed=1;
				UART_Printf("All Axis Homed<OK>\n>>");	
