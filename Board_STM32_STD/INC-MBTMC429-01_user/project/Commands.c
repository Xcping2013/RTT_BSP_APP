...
uint8_t  ProcessCommand(char  *Commands)
{
	...
	else	if(KEY_RESET_pressed == 0 && CommmandLine_rotate(Commands)==1)							return 1;		
	else	if(KEY_RESET_pressed ==0 && CommmandLine_move(Commands)==1)								return 1;
...
//Project switch	
	else if(strcmp("type[basic]",	Commands)==0)					{		AT24CXX_WriteOneByte(ADDR_PROJECT_ID,BASIC_APP);			UART_SendStr("OK\n");					return 1;		}		
	...
	else if(strcmp("type[road]",		  Commands)==0)				{	AT24CXX_WriteOneByte(ADDR_PROJECT_ID,BUTTON_ROAD);		UART_SendStr("OK\n");					return 1;		}			
...
{	
	...
	if( ProID!= PROJECT_NONE  && ProID!=JABIL_LCR )
	{
		UART_SendStr("\nread motion status:\n");	
...
void board_debug(void)
{
	...
	hw_in_out_debug();
	hw_eeprom_debug();
	if( ProID==BUTTON_ONLINE || ProID==BUTTON_OFFLINE || ProID==BUTTON_VER3 || ProID==LIDOPEN || BASIC_APP || ProID==BUTTON_ROAD)
	{
		hw_motor_debug();
	}
}
