...
{
	u32 SizeLeft=Size;						//Byte left to write
	u32 DataOffset = 0;						//current offset in data pointer
	u32 PageOffset;								// current offset in page
	u32 ByteNext	 = 0;						// next n bytes to write

	// write all bytes in multiple steps
	while (SizeLeft > 0)
	{
		// calc offset in page
		PageOffset = Address % PAGE_SIZE_EEPROM;
		// maximal 30 bytes to write
		ByteNext = min(min(SizeLeft, PAGE_SIZE_EEPROM), PAGE_SIZE_EEPROM - PageOffset);
